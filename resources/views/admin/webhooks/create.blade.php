...
@section('title', 'Create Webhook - Admin')
@section('header', 'Create Webhook')
    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-8">
        <form method="POST" action="{{ route('admin.webhooks.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1">The URL that will receive POST requests when events occur.</p>
            </div>
            ...
            <div>
                ...
                <input id="secret" type="text" name="secret" value="{{ old('secret') }}"
                ...
                <p class="text-xs text-gray-400 mt-1">If set, a SHA256 HMAC signature will be sent in the <code class="bg-gray-100 px-1 rounded">X-Webhook-Signature</code> header.</p>
            </div>
                ...
                <div class="space-y-2">
                    <label class="flex items-center space-x-3">
                        <input type="checkbox" name="events[]" value="order.paid"
                               ...
                               {{ in_array('order.paid', old('events', [])) ? 'checked' : '' }}>
                    ...
                    <label class="flex items-center space-x-3">
                        <input type="checkbox" name="events[]" value="user.registered"
                               class="rounded border-gray-300 text-[#FFC300] focus:ring-[#FFC300]"
                               {{ in_array('user.registered', old('events', [])) ? 'checked' : '' }}>
                    ...
                    <label class="flex items-center space-x-3">
                        <input type="checkbox" name="events[]" value="user.verified"
                               class="rounded border-gray-300 text-[#FFC300] focus:ring-[#FFC300]"
                               {{ in_array('user.verified', old('events', [])) ? 'checked' : '' }}>
